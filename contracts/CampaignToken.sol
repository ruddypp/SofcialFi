// SPDX-License-Identifier: MIT
pragma solidity ^0.8.30;

import "@openzeppelin/contracts/token/ERC20/ERC20.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

/**
 * @title CampaignToken
 * @dev ERC-20 token untuk membuat campaign gratis
 * Token ini diberikan sebagai reward dan dapat digunakan untuk membuat petition tanpa bayar ETH
 */
contract CampaignToken is ERC20, Ownable {
    address public petitionContract;
    address public soulboundMemberContract;
    
    event PetitionContractUpdated(address indexed newContract);
    event SoulboundMemberContractUpdated(address indexed newContract);
    
    constructor() ERC20("Campaign Token", "CAMP") Ownable(msg.sender) {}
    
    /**
     * @dev Set address PetitionPlatform contract
     * Hanya bisa dipanggil oleh owner setelah PetitionPlatform di-deploy
     */
    function setPetitionContract(address _petitionContract) external onlyOwner {
        require(_petitionContract != address(0), "Invalid address");
        petitionContract = _petitionContract;
        emit PetitionContractUpdated(_petitionContract);
    }
    
    /**
     * @dev Set address SoulboundMember contract
     * Diperlukan agar SoulboundMember bisa memanggil mint() untuk token gratis
     */
    function setSoulboundMemberContract(address _soulboundMemberContract) external onlyOwner {
        require(_soulboundMemberContract != address(0), "Invalid address");
        soulboundMemberContract = _soulboundMemberContract;
        emit SoulboundMemberContractUpdated(_soulboundMemberContract);
    }
    
    /**
     * @dev Mint token baru
     * Hanya bisa dipanggil oleh PetitionContract, Owner, atau SoulboundMember Contract
     */
    function mint(address to, uint256 amount) external {
        require(
            msg.sender == petitionContract || 
            msg.sender == owner() || 
            msg.sender == soulboundMemberContract,
            "Not authorized"
        );
        _mint(to, amount);
    }
    
    /**
     * @dev Burn token dari address tertentu
     * Hanya bisa dipanggil oleh PetitionContract
     */
    function burnFrom(address from, uint256 amount) external {
        require(msg.sender == petitionContract, "Not authorized");
        _burn(from, amount);
    }
}